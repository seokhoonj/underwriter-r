...
list_rule_impact <- function(applied, combined, coverage = NULL) {
  ...
  # cells that are on manual review in the combined -- the only ones that can flip
  ...
  review_cells <- combined_long[decision == manual_review, .(id, coverage)]
  ...
  matched_review <- melt(applied_dt[matched == 1L], id.vars = c("id", "kcd_main"),
  ...
  unmatched_review <- applied_dt[matched == 0L, .(id, kcd_main)][, .(coverage = decision_cols),
                                                                by = .(id, kcd_main)]
