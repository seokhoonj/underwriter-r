...
#' marginals miss).
#'
#' The combined and marginal lifts are exact re-combines via [simulate_relaxation()], so
...
#'   for regex families relax them with [simulate_relaxation()] directly.
#' @param mode Passed to [simulate_relaxation()]: `"review_only"` (default) or `"full"`.
...
#'   [screen_relaxation()] to rank diseases individually.
#' @export
decompose_relaxation <- function(applied, final, kcd_main, mode = c("review_only", "full")) {
  mode    <- match.arg(mode)
