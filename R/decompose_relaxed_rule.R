...
#' @export
decompose_relaxed_rule <- function(applied, final, kcd_main) {
  targets <- unique(kcd_main)
  if (length(targets) < 2L)
    stop("`kcd_main` must name at least two representative codes to relax together.")

  # denominator = evaluated (non-empty) decision cells, matching list_rule_impact()
  ...
  flips <- function(regex) sum(relax_rule(applied, final, regex)$n_flipped)
